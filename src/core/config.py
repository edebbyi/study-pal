...
def _read_setting(name: str, default: str, secrets: dict[str, str]) -> str:
    ...
    """
    return os.getenv(name, secrets.get(name, default))  # env overrides secrets for local testing
...
class Settings:
    ...
    def load(cls) -> "Settings":
        ...
        return cls(
            ...
            langfuse_prompt_answer=_read_setting("LANGFUSE_PROMPT_ANSWER", "study_pal_answer", secrets),
            ...
            langfuse_prompt_quiz=_read_setting("LANGFUSE_PROMPT_QUIZ", "study_pal_quiz", secrets),
            langfuse_prompt_reteach=_read_setting("LANGFUSE_PROMPT_RETEACH", "study_pal_reteach", secrets),
            langfuse_prompt_study_plan=_read_setting("LANGFUSE_PROMPT_STUDY_PLAN", "study_pal_study_plan", secrets),
            langfuse_prompt_follow_up=_read_setting(
                "LANGFUSE_PROMPT_FOLLOW_UP",
                "study_pal_follow_up",
                secrets,
            ),
            langfuse_prompt_version=_read_setting("LANGFUSE_PROMPT_VERSION", "", secrets),
            rerank_model=_read_setting("OPENROUTER_RERANK_MODEL", "", secrets),
            rerank_candidates=int(_read_setting("OPENROUTER_RERANK_CANDIDATES", "12", secrets)),
        )
